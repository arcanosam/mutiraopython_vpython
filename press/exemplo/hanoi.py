...
while True:
    rate(100)
    if self.dsy.mouse.events: # scene.mouse.events:
        m = self.dsy.mouse.getevent() # scene.mouse.getevent()
        if m.drag:  # identify pole clicked on
            ...
            if len(stack[pole1])>0:
                ...
                del(stack[pole1][-1])
                while not (self.dsy.mouse.events and self.dsy.mouse.getevent().drop): # scene.mouse.events and scene.mouse.getevent().drop):      # drag ring
                    select.pos=self.dsy.mouse.project(normal=vector(0,0,1)) # scene.mouse.project(normal=vector(0,0,1))
                    rate(60)
                mx=select.x
